(() => {
  ...
  const Epic = window.LS77Epic || window.LS76Epic || window.LS75Epic || {storyForChapter:()=>null,trialForChapter:()=>null,branchEnding:()=>null,introForChapter:()=>'',isEpic:()=>false,count:0};
  const Rogue = window.LS77Rogue || window.LS76Rogue || window.LS75Rogue || null;
  ...
  const Balance = window.LS77Balance || window.LS76Balance || {get:()=>({factor:1,band:'正常'}),applyKit:x=>x,applyStats:x=>x,simulate:null};
  const Ops = window.LS77Operations || window.LS76Operations || {unlocked:()=>true,portrait:()=>'',background:()=>'',freshTutorial:()=>({completed:true,step:0,seen:[]}),freshLoadouts:()=>[],economyCap:()=>24,fatigueRecover:()=>0};
  ...
  const freshState = () => ({
    ...
    dispatches:[],
    tutorial:Ops.freshTutorial?.()||{completed:false,step:0,seen:[]},equipmentPlans:Ops.freshLoadouts?.()||[],operations:{deployment:null,lastBalanceAt:'',economy:{lastFatigueAt:now()}},
    endgame:{towerFloor:1,towerBest:0,expedition:null,rogue:null,weekly:{},bossRecords:{},suspendedRun:null},
  ...
  function resolveChoiceEvent(){
    ...
    openModal(`${trial.icon} ${trial.name}`,`<p>${esc(trial.prompt)}</p><div class="choice-grid">${trial.options.map((o,i)=>`<button class="choice-card" data-modal="trial-choice" data-trial-index="${i}"><b>${String.fromCharCode(65+i)}．${esc(o)}</b></button>`).join('')}</div><p class="muted">此判斷會改變後續敵情、獎勵或章回評分。</p>`);
  }
  function resolveTrial(index){
    ...
    const i=clamp(Number(index)||0,0,trial.options.length-1);run.trialKey=trial.key;run.trialChoice=i;run.trialSuccess=i===trial.best;run.trialResult=run.trialSuccess?trial.success:trial.fail;run.trialEffect=trialEffect(trial.key,run.trialSuccess);run.choiceEventDone=true;
    ...
    run.log.unshift(`${trial.name}：選擇「${trial.options[i]}」。${run.trialResult}`);closeModal();save(true);renderChapter();toast(run.trialSuccess?'判斷正確，獲得戰術優勢。':'判斷失誤，仍可在戰鬥中挽回。',run.trialSuccess?'good':'warn');tone('skill');
  }
  ...
  function nav(){const items=[['home','首頁'],['chapters','章回'],['team','編隊'],['heroes','英雄'],['forge','裝備'],['base','山寨'],['endgame','遠征'],['cloud','雲端']];return`<nav class="nav" aria-label="主要導覽">${items.map(([key,label])=>{const ok=featureUnlocked(key);const need=Ops.featureRules?.[key]||0;return`<button class="btn small ${ok?'':'locked-nav'}" data-act="${key}" ${ok?'':`aria-disabled="true" title="完成 ${need} 回後開放"`}>${label}${ok?'':` 🔒`}</button>`;}).join('')}<span class="nav-spacer"></span><span class="tag">銀兩 ${state.silver}</span><span class="tag">完成 ${completionCount()}/108</span></nav>`;}
  ...
  function render(){
    document.body.dataset.theme=prefs.theme;A11y.apply(prefs);
    if(screen==='home')renderHome();else if(screen==='chapters')renderChapters();else if(screen==='chapter')renderChapter();else if(screen==='battle')renderBattle();else if(screen==='ending')renderEnding();else if(screen==='team')renderTeam();else if(screen==='heroes')renderHeroes();else if(screen==='forge')renderForge();else if(screen==='base')renderBase();else if(screen==='endgame')renderEndgame();else if(screen==='cloud')renderCloud();
  ...
  function openReleaseNotes(){openModal('v7.7.0 更新內容',`<ul><li>108 名英雄第一輪正式平衡調整與 12 場景模擬</li><li>36 個重要章回手工劇情、多重結局與章回背景</li><li>分岔 Rogue-like 路線、36 件遺物與每週挑戰碼</li><li>雲端欄位合併預覽、版本歷史與還原前備份</li><li>互動式新手教學與功能分階段解鎖</li><li>裝備方案、批次鎖定與自動分解準備</li><li>派遣疲勞、離線收益動態上限與經濟平衡</li><li>36 天罡水墨角色卡、章回背景與程序式中國風音樂</li><li>公開部署營運診斷與三平台 PWA 驗收清單</li></ul><div class="actions"><button class="btn" data-modal="close">關閉</button></div>`);}
