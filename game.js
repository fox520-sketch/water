(() => {
  ...
  const freshState = () => ({
    ...
    chain:{version:'7.9.0',flags:{},log:[],perfect:0,renown:0,lastApplied:{}},
  ...
  function exportOpsReport(kind){
    let report=null;
    if(kind==='issue'){report=Stability79?.issueReport?.(state,prefs);state.operations.issueReports=state.operations.issueReports||[];state.operations.issueReports.unshift(report);state.operations.issueReports=state.operations.issueReports.slice(0,20);}
  ...
  function closeModal(){modalRoot.innerHTML='';}
    function openReleaseNotes(){openModal('v7.9.0 更新內容',`<ul><li>一鍵更新檢查頁、版本相容性與 Service Worker 快取診斷</li><li>更新失敗回復包與 rollback 說明</li><li>存檔遷移報告、玩家問題回報工具</li><li>GitHub Pages 與 Firebase 跨裝置實測報告匯出</li><li>章回連鎖影響系統：完美結局會累積聲望與旗標</li><li>遠征賽季、遺物套組與個人排行榜</li><li>v7.8.0 更新包仍可回復，適合正式營運前穩定更新</li></ul><div class="actions"><button class="btn" data-modal="close">關閉</button><button class="btn primary" data-act="ops">前往維護中心</button></div>`);}
