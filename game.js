(() => {
  ...
  const Epic = window.LS76Epic || window.LS75Epic || {storyForChapter:()=>null,trialForChapter:()=>null,branchEnding:()=>null,introForChapter:()=>'',isEpic:()=>false,count:0};
  const Rogue = window.LS76Rogue || window.LS75Rogue || null;
  ...
  const Balance = window.LS76Balance || {get:()=>({factor:1,band:'正常'}),applyKit:x=>x,applyStats:x=>x,simulate:null};
  const Ops = window.LS76Operations || {unlocked:()=>true,portrait:()=>'',background:()=>'',freshTutorial:()=>({completed:true,step:0,seen:[]}),freshLoadouts:()=>[],economyCap:()=>24,fatigueRecover:()=>0};
